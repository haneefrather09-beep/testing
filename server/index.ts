...
const DIPLOY_HEADER_KEY = "X-Powered-By";
const DIPLOY_HEADER_VALUE = "Whatsway";
const DIPLOY_VERSION = "1.0.0";
const DIPLOY_PRODUCT_NAME = "Whatsway";
const diployLoggerBase = { 
    info: (...args: any[]) => console.log("[INFO]", ...args), 
    error: (...args: any[]) => console.error("[ERROR]", ...args), 
    warn: (...args: any[]) => console.warn("[WARN]", ...args),
    banner: (msg: string) => console.log(`\n=== ${msg} ===\n`)
};
const diployLogger: any = new Proxy(diployLoggerBase, {
    get: (target: any, prop: string) => {
        if (prop in target) return target[prop];
        return (...args: any[]) => console.log(`[${prop.toUpperCase()}]`, ...args);
    }
});
...
if (process.env.DATABASE_URL && process.env.DATABASE_URL !== "dummy") {
  ...
  console.log("[Session] Using Memory session store (sessions will be lost on restart)");
}
...
(async () => {
  ...
  try {
    ...
    if (process.env.DATABASE_URL && process.env.DATABASE_URL !== "dummy") {
        ...
        console.log("[Startup] Skipping migrations (using MemStorage)");
    }
  } catch (err) {
  ...
  httpServer.listen(listenOptions, async () => {
    diployLogger.banner("Whatsway PLATFORM STARTUP");
