...
getAnalyticsByChannel(channelId: string, days?: number): Promise<Analytics[]>;
  ...
  getPlansById(id: string): Promise<any | undefined>;
...
async searchContactsByChannel(channelId: string, query: string): Promise<Contact[]> {
  ...
  async getPlansById(id: string): Promise<any | undefined> {
    return this.plans.get(id);
  }

  async seed() {
    console.log("🌱 Seeding MemStorage...");
    ...
    if (!existingAdmin) {
      ...
      console.log("  ✅ Created superadmin (Superadmin@123)");
    }

    // Add Default Plans
    if (this.plans.size === 0) {
      const defaultPlans = [
        { id: "free", name: "Free", monthlyPrice: "0", annualPrice: "0", permissions: { channel: "1", contacts: "500" } },
        { id: "pro", name: "Professional", monthlyPrice: "29", annualPrice: "290", permissions: { channel: "3", contacts: "5000" } }
      ];
      for (const plan of defaultPlans) this.plans.set(plan.id, plan);
...
// Use DatabaseStorage with real PostgreSQL
